...
// extends the same check to the remaining static Program content, which
// also shipped one fabricated credentialed identity ("Dr. Emily Roberts,
// PhD" on the mindful-eating-workbook resource — the /resources feature
// that resource lived under was later retired entirely, see Issue #25 and
// __tests__/resources-route.test.ts).
...
describe("no fabricated credentialed identities in user-facing content (Issue #15)", () => {
  ...
  it("lib/mdx.ts no longer exports the retired Resource feature", async () => {
    const mdxModule = await import("@/lib/mdx")
    expect((mdxModule as Record<string, unknown>).resources).toBeUndefined()
    expect((mdxModule as Record<string, unknown>).getResources).toBeUndefined()
    expect((mdxModule as Record<string, unknown>).getResource).toBeUndefined()
  })
